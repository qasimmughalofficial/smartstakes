...
function BettingListView(props) {
    ...
    function dataRow() {
        return data.map((match, index) => {
            return (
            <NavLink to={`/app/${currentSport}/${match.match_id}`} className="link-anchor"  key={index} onClick={() => linkHandler(match)}>
                {/* <h5 className='list-date'>{match.disp_date}</h5> href={`/app/matchDetail/sport=${currentSport}&matchId=${match.match_id}`} */}
                {duplicateDate(match.disp_date)}
                {/* <div className="row-match desktop-view">
                    <div className="upper-row-match same-row-style">
                        ...
                        <div className='column-six same-col-style'>
                            ...
                            <p>
                                {match.odds} at <img src={`https://Rugby4Cast.b-cdn.net/bookies/${match.bookie}.jpg`} alt='...'/>
                            </p>
                        </div>
                    </div>
                    <div className="lower-row-match same-row-style">
                        <div className='column-seven same-col-style'>
                            <p><span className='basic-name'>{match.league}</span></p>
                        </div>
                        <div className='column-eight same-col-style'>
                            <p>{match.opp.replace(/_/g, ' ')}</p>
                        </div>
                        <div className='column-nine same-col-style'>
                            <p>{match.pred_perc_opp}%</p>
                        </div>
                        <div className='column-ten same-col-style'>
                            <p>{match.pred_score_opp} GOALS</p>
                        </div>
                        <div className='column-eleven same-col-style'>
                            <ul className='scoreboard-ul'>
                        ...
                        <div className='column-telve same-col-style'>
                            <p>
                                {match.odds_opp} at <img src={`https://Rugby4Cast.b-cdn.net/bookies/${match.bookie_opp}.jpg`}  alt='...'/>
                            </p>
                        </div>
                    </div>
